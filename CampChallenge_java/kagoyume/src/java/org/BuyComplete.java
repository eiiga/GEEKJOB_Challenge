...
public class BuyComplete extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try  {
            ...
            int type = Integer.parseInt(request.getParameter("transport"));
            ArrayList<SearchData> cartitem = (ArrayList<SearchData>)session.getAttribute("cartitem");
            UserDataDTO user = (UserDataDTO)session.getAttribute("userinfo");
            UserDataDTO buy = new UserDataDTO();
            
            for(int i = 0; i < cartitem.size(); i++){
                    buy.setUserID(user.getUserID());
                    buy.setItem(cartitem.get(i).getCode());
                    buy.setType(type);
                    UserDataDAO .getInstance().insertbuy(buy);
                    buy = new UserDataDTO();
            }
            ...
            buy.setUserID(user.getUserID());
            buy.setTotal(total);
            UserDataDAO .getInstance().updatetotal(buy);
